...
public class KeyValueStore {
    ...
    public HashMap<Integer, KeyValueStore> repartition(int partitionId, int partitionCount) {

        synchronized(keyStore)
        {

            HashMap<Integer, KeyValueStore> result = new HashMap<Integer, KeyValueStore>();

            for(String key : keyStore.keySet())
            {
                int id = keyStore.getPartitionId;
                //rehash keys
                
                if( != id) //check if belongs to partition (use id)
                {
                    //add it to hashmap
                }
